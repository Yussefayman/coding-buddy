...
class MemoryTool(Tool):
    name = 'memory'
    description = 'Store and retrieve persistent memory. Use this to remember user preferences, important context and notes'
    ...
    async def execute(self, invokation: ToolInvokation) -> ToolResult:
        params = MemoryParams(**invokation.params)
        ...
        if params.action.lower() == 'set':
            ...
            memory = self._load_memory()
            if params.key not in memory.get('entires', {}):
