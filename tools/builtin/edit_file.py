...
from tools.base import FileDiff, Tool, ToolInvokation, ToolKind, ToolResult
from pydantic import BaseModel, Field
from utils.paths import ensure_parent_directory, resolve_path

class EditParams(BaseModel):
    path: str = Field(..., description='Path to the file to edit(relative to working directory or absoulte path)')
    old_string: str = Field(
        "",description= 'The exact text to find and replace. it must match exactly including all whitespace and indentation. for new files, leave this empty.')
...
class EditFileTool(Tool):
    ...
    async def execute(self, invokation: ToolInvokation) -> ToolResult:
        params = EditParams(**invokation.params)
        path = resolve_path(invokation.cwd, params.path)

        if not path.exists():
            if params.old_string:
                return ToolResult.error_result(f'File does not exists: {path}. To create a new file, use an empty old_string.')
        ...
        if not params.old_string:
            return ToolResult.error_result(
                'old string is empty but file exists. Provide old_string to edit, or ise write_file to overwrite.'
            )
        occurrence_count = old_content.count(params.old_string)
        if occurrence_count == 0:
            return self._no_match_error(params.old_string, old_content, path)
        
        if occurrence_count > 1 and params.replace_all:
            return ToolResult.error_result(
        ...
        if new_content == old_content:
            return ToolResult.error_result('No change made - old stirng equals new_string')
        ...
        return ToolResult.success_result(
            f'Edited {path}: replaced {replace_count} occurrences(s)',
            diff = FileDiff(
                path=path,
