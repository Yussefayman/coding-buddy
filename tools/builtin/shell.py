...
class ShellTool(Tool):
    name = 'shell'
    kind = ToolKind.SHELL
    description = 'Execute a shell command. Use this for running system commands, script and CLI tools.'
    schema = ShellParams

    async def execute(self, invokation: ToolInvokation) -> ToolResult:
        params = ShellParams(**invokation.params)
        command = params.command.lower().strip()
        
        for blocked in BLOCKED_COMMANDS:
            if blocked in command:
                return ToolResult.error_result(f'Command Blocked for safety: {params.command}',
