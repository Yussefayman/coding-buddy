...
class ReadFileParams(BaseModel):
    path: str = Field(
        ..., description='Path to the file to read (related to wd or absolute path)'
    )
    ...
    async def execute(self, invokation: ToolInvokation) -> ToolResult:
        ...
        try:
            ...
            if params.limit is not None:
                end_indx = min(start_idx+params.limit, total_lines)
            else:
                end_idx = total_lines
            ...
            return ToolResult.error_result(f"Failed to read the file {e}")
